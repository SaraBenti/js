...
function StampaNumeriPrimi(n) {
  //stampa solo fino a n e non per n volte
  if (typeof n === "number" && n >= 1) {
    for (let i = 1; i <= n; i++) {
      let risultato = 1;
      if (isNumeroPrimo(i)) {
...
garage.rimuoviVeicolo = function (automobile) {
  if (this.validaVeicolo) {
    ...
    for (let i = 0; i <= this.auto.length; i++) {
      ...
      if (
        autoInGarage.marca === this.auto.marca &&
...
/**
 * CREARE LE CLASSI GARAGE E AUTO
 */
class Garage {
  constructor() {
    this.automobili = [];
  ...
  indiceAuto(auto) {
    ...
    for (let i = 0; i < this.automobili.length; i++) {
      const macchina = this.automobili[i];
  ...
  inserisciAuto(auto) {
    if (this.validaAuto(auto) && this.indiceAuto < 0) {
      this.automobili.push(auto);
    }
  }

  rimuoviVeicolo(auto) {
    let indice = this.indiceAuto(auto);
    if (this.validaAuto(auto)) {
...
//slice (indice,1) elimina un elemento e l'indice per l'elemento successivo cambia perchè
//prende il posto di quello
//precedente e quindi slice evita buchi negli indici
//io posso navigare l'array di automobili mentre in una classe devo richiedere specificatamente
//l'auto ma la classe è un contenitore
//di proprietà
...
garage.inserisciAuto(auto);
garage.inserisciAuto(auto1);
garage.inserisciAuto(auto2);
